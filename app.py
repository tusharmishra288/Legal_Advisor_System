...
# Injection of JS for Auto-Scroll
...
st.markdown("""
    ...
    /* Clean Chat Bubble Architecture (No horizontal lines) */
...
# --- 3. System Core (Neon + Auto-Schema Logic) ---
@st.cache_resource
def init_system_core():
    ...
    with pool.connection() as conn:
        ...
        with conn.cursor() as cur:
            ...
            # Schema Migration for message_id persistence
...
def run_research_logic(prompt, is_regen=False):
    ...
    with st.chat_message("assistant"):
        placeholder = st.empty(); streamed = ""
        for word in advice_buffer.split(' '):
            streamed += word + " "; placeholder.markdown(streamed + "▌"); time.sleep(0.01)

        # Immediate Capture of Message ID for Audit Card Visibility
...
# --- 6. Sidebar Implementation (Zero-Artifact Build) ---
with st.sidebar:
...
if not st.session_state.busy:
    ...
    if "graph_state" not in st.session_state or not st.session_state.graph_state.get("messages"):
        try: st.session_state.graph_state = graph.get_state(config_render).values
        except: st.session_state.graph_state = {"messages": []}
    
    raw = st.session_state.graph_state.get("messages", [])
    history, ai_buf = [], []
    for m in raw:
        if isinstance(m, HumanMessage):
            if ai_buf: history.append(ai_buf[-1])
            ai_buf, history = [], history + [m]
        elif isinstance(m, AIMessage):
            if not any(x in m.content.lower() for x in ["penalty:", "verdict:"]): ai_buf.append(m)
    if ai_buf: history.append(ai_buf[-1])
        ...
        for i, msg in enumerate(history):
            with st.chat_message("user" if isinstance(msg, HumanMessage) else "assistant"):
                ...
                if isinstance(msg, AIMessage):
                    # Audit Persistence Logic (ID-Locked)
                    msg_id = getattr(msg, 'id', None)
                    if msg_id and msg_id in audit_history:
                        audit = audit_history[msg_id]; s, f = audit["score"], audit["feedback"]
                        if s >= 8: status_lbl, cls, info = "HIGH CONFIDENCE", "fid-high", "Direct statutory match."
                        elif s >= 5: status_lbl, cls, info = "MODERATE MATCH", "fid-med", "Verify specific sub-sections."
                        else: status_lbl, cls, info = "ACTION: TRIGGER REGENERATE", "fid-low", "Low fidelity detected. Please click the button below."
